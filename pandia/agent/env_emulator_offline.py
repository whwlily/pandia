...
from pandia.agent.q_network import q_network, q_ensemble, gmm_policy
...
class WebRTCEmulatorEnv_offline(WebRTCEnv):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.step_count == 0:
            self.start_webrtc(bw=self.net_config["bw_file_name"], delay=self.net_config["delay"], loss=self.net_config["loss"], jitter=self.net_config["jitter"])
        ts = time.time()
        ...
        if self.print_step and time.time() - self.last_print_ts > self.print_period:
            ...
            # 展示
            self.log(f'#{self.step_count}@{int((time.time() - self.start_ts))}s, '
                    f'{act}{"(C)" if action_capped else ""}, '
                    f'True capacity: {self.obs_thread.cur_capacity / K:.02f}Mbps, '
                    f'{self.observation}')
...
def test_single(trace_file, is_gcc_model, factor_range):
    ...
    try:
        env.reset()
        bitrates = [0 * K] if net_config['is_gcc'] else [200 * K]
        observation = None
        pre_act = 200 * K
        counter = 5
        for i, bitrate in enumerate(bitrates):
            ...
            # cur_act = min(max(predict, 200 * K), 4 * M)
            # q_mean_val, q_std_val = q_ensemble(q_net, observation, bw_prediction[0,0,0])
            # predict = observation[5] * np.exp(bw_prediction[0,0,0] - 0.1 * abs(bw_prediction[0,0,1])) if bw_prediction[0,0,0] > 0 else observation[5] * np.exp(bw_prediction[0,0,0] + 0 * abs(bw_prediction[0,0,1]))
            ...
            # cur_act = min(max(predict, 100 * K), 8 * M) if observation[0] != 0 and observation[35] < 200 else 0
            # cur_act = min(max(predict, 200 * K), 8 * M) if cur_slope < 4 and np.sqrt(q_var_val) < 100 else 0
            # alpha = 0.08 if i < 100 else 0.04
            
            # GMM policy
            mean, std, pi, _, _ = orts.run(None, feed_dict)
            # 对于每个样本，找到权重最大的分支索引
            # (mean, std, pi), _, _ = actor(
            #     torch.from_numpy(observation.reshape(1,1,-1)), 
            #     torch.zeros((1, 1)), 
            #     torch.zeros((1, 1))
            # )
            ...
            # print(f"selected_actions: {selected_actions}, selected_std: {selected_std}")
            ...
            # alpha = 0.05
            # if i < 200:
            #     if predict > pre_act * 2 and q_std_val > q_mean_val * alpha * 2:
            #         predict =  pre_act * 1.4
            # else:
            #     if predict > pre_act * 1.5 and q_std_val > q_mean_val * alpha:
            #         predict =  pre_act * 1.1

            # print(f"_std_val/q_mean_val: {q_std_val/q_mean_val:.04f}, q_std_val: {q_std_val:.02f}, q_mean_val: {q_mean_val:.02f}")
            # alpha = 0.08
            # max_act = min(pre_act * 1.2, 1.2 * M) if pre_act > 400 * K else min(pre_act * 2, 1.2 * M)
            # min_act = max(pre_act * 0.6, 200 * K)
            # q_mean_val = abs(q_mean_val)
            # if predict > 400 * K:
            #     if i < 100 and q_std_val < q_mean_val * alpha * 2:
            #         cur_act = min(max(predict, min_act), max_act)
            ...
            # else:
            #     cur_act = min(max(predict, min_act), max_act)
            cur_act = min(max(predict, 200 * K), 1.2 * M)
