...
def test_single():
    ...
    config['gym_setting']['logging_path'] = '/tmp/pandia_test_offline.log'
    ...
    try:
        ...
        model_path = "/data2/kj/Workspace/Pandia/pandia/model/checkpoint_580000.onnx"
        ...
        for i, bitrate in enumerate(bitrates):
            ...
            # action.bitrate = max(min(bitrate, max_bw), min_bw)
            ...
            if i >= 49:
                bitrates.append(bw_prediction[0,0,0])
            # bitrates.append(bw_prediction[0,0,0])
            print(f'{i}, 预测值: {bw_prediction[0,0,0] / M:.02f} Mbps')
            # if i >= 9:
            #     bitrates.append(6*M)
            if i == 1000:
                break
        ...
        pass
    env.close()
    fig_path = os.path.join(RESULTS_PATH, 'env_emulator_test_offline_step_60ms_test')
    # fig_path = os.path.join(RESULTS_PATH, 'pacing_with_bitrate','env_emulator_test_offline_step_600ms')
    # fig_path = os.path.join(RESULTS_PATH, 'pacing_rate','env_emulator_test_offline_step_600ms')
